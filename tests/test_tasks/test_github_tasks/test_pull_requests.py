    ...
    try:
        with httpx.Client() as client:
            ...
            with test_db_session.engine.connect() as connection:
                ...
                result = connection.execute(f"SELECT * FROM data.pull_requests;").fetchall()
         ...
         with test_db_session.engine.connect() as connection:
            ...
            connection.execute(f"DELETE FROM data.pull_requests;")
            connection.execute("""DELETE FROM "data"."repo";
                                DELETE FROM "data"."repo_groups";
                                """)
            connection.execute(f"DELETE FROM data.contributors WHERE cntrb_id!='{not_provided_cntrb_id}' AND cntrb_id!='{nan_cntrb_id}';")
