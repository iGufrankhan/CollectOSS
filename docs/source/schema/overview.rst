...
The latest version of CollectOSS includes a schema_ that brings together data around key artifacts of open source software development.
...
data. Note that all messages are stored in CollectOSS in the ``messages`` table.
This is to facilitate easy analysis of the tone and characteristics of text
communication in a project from one place.

2. ``collectoss.tasks.git.facade_tasks``: Based on
...
4. ``collectoss.tasks.github.pull_requests.tasks``: Collects Pull Request related data such as commits, contributors,assignees, etc. from the Github API and stores it in the CollectOSS database.
