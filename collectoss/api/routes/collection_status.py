...
@app.route('/{}/collection_status/commits'.format(API_VERSION))
...
@app.route('/{}/collection_status/issues'.format(API_VERSION))
        ...
        FROM
            (
                ...
                FROM
                    ...
                    data.issues d,
                    data.repo_info b,
                    ( SELECT repo_id, MAX ( data_collection_date ) AS last_collected FROM data.repo_info GROUP BY repo_id ORDER BY repo_id ) e,
...
@app.route('/{}/collection_status/pull_requests'.format(API_VERSION))
def pull_request_collection_status():  # TODO: make this name automatic - wrapper?
    pull_request_collection_sql = s.sql.text("""
        ...
        FROM
            ...
            FROM
                data.repo A,
                data.pull_requests d,
                data.repo_info b,
                ( SELECT repo_id, MAX ( data_collection_date ) AS last_collected FROM data.repo_info GROUP BY repo_id ORDER BY repo_id ) e,
                ( SELECT repo_id, MAX ( data_collection_date ) AS last_pr_collected FROM data.pull_requests GROUP BY repo_id ORDER BY repo_id ) f
            WHERE
                A.repo_id = b.repo_id
                AND LOWER ( A.repo_git ) LIKE'%github.com%'
                AND A.repo_id = d.repo_id
